...
if env_file.exists():
    ...
    updated = updated.replace(
        ...
        'if False and WEBUI_NAME != "Open WebUI":\n    WEBUI_NAME += ""',
    )
    updated = updated.replace(
...
if db_file.exists():
    ...
    try:
        ...
        if row:
            ...
            data = {"version": 0, "ui": {"enable_signup": False, "prompt_suggestions": []}}
            con.execute(
        ...
        for user_id, raw_settings in con.execute("select id, settings from user").fetchall():
            ...
            ui["language"] = "ru-RU"
            con.execute(
                "update user set name = ?, settings = ?, updated_at = strftime('%s', 'now') where id = ?",
                ("Пользователь", json.dumps(settings, ensure_ascii=False), user_id),
            )
        con.commit()
