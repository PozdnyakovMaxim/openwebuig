...
Для корректной загрузки DOCX контейнер должен использовать внешний конвертер этого же
RAG API. В `deploy/env/openwebui.env` должны быть заданы:

```text
CONTENT_EXTRACTION_ENGINE=external
EXTERNAL_DOCUMENT_LOADER_URL=http://host.docker.internal:8000
EXTERNAL_DOCUMENT_LOADER_API_KEY=тот_же_секрет_что_OPENAI_COMPAT_API_KEY
```

Внешний endpoint `/process` читает OOXML напрямую и восстанавливает автоматическую
Word-нумерацию из `numbering.xml`. Когда Open WebUI передаёт извлечённые фрагменты модели,
API отвечает непосредственно по ним и не запускает второй поиск в корпоративном pgvector.
Это разделяет два сценария: постоянный корпоративный корпус ищется через BGE-M3/pgvector,
а прикреплённый к чату файл обрабатывается как контекст текущего файла.
Тело загрузки читается с потоковым лимитом, DOCX проверяется на повреждённый OOXML,
опасный размер распаковки и подозрительное ZIP-сжатие до разбора.

Если в `~/openwebuig/.env` задан отдельный `OPENWEBUI_DOCUMENT_LOADER_API_KEY`, в
`EXTERNAL_DOCUMENT_LOADER_API_KEY` Open WebUI нужно указать именно его. При пустом
отдельном ключе используется `OPENAI_COMPAT_API_KEY`.

Если контейнер уже существует, после обновления кода применить настройку в persistent
конфигурации Open WebUI с автоматическим backup `webui.db`:

```bash
sudo bash scripts/repair_openwebui_access.sh
```

Файлы, загруженные в Open WebUI до включения внешнего конвертера, нужно загрузить заново:
старые сохранённые чанки автоматически не пересчитываются. Сейчас внешний конвертер
поддерживает DOCX и текстовые UTF-8 файлы; неподдерживаемый бинарный формат получает `415`.
...
Проверить конвертер на реальном DOCX, содержащем автоматическую Word-нумерацию:
...
source .env
set +a
TEST_DOCX=/полный/путь/к/проверочному.docx
curl -fsS -X PUT http://127.0.0.1:8000/process -H "Authorization: Bearer $OPENAI_COMPAT_API_KEY" -H "Content-Type: application/vnd.openxmlformats-officedocument.wordprocessingml.document" -H "X-Filename: $(basename "$TEST_DOCX")" --data-binary "@$TEST_DOCX" | python3 -m json.tool
```

В `page_content` должны присутствовать видимые номера пунктов, а в metadata —
`"word_numbering_preserved": true`. После этого загрузить тот же файл заново в новый чат
Open WebUI и спросить точный номер пункта. Старый загруженный экземпляр для этой проверки
не использовать.
...
`webui.db`, затем обновляет persistent connection, модель по умолчанию, доступ и внешний
DOCX-конвертер (`/process`). Он
...
На рабочем Ubuntu весь безопасный цикл (backup, schema, candidate, strict audit,
атомарная замена, restart и health-check) можно запустить одной командой. По умолчанию
она ожидает 41 DOCX в `~/rag_template/docs`:

```bash
./reindex
```

При другом каталоге или количестве документов передать `DOCS_DIR` и
`EXPECTED_DOCUMENTS` через окружение.
...
OPENWEBUI_DOCUMENT_MAX_BYTES=67108864
OPENWEBUI_CONTEXT_MAX_CHARS=60000
RAG_RETRIEVAL_LIMIT=6
RAG_DOCUMENT_RESULT_LIMIT=12
RAG_DOCUMENT_MIN_VECTOR_SCORE=0.45
RAG_DOCUMENT_MIN_TEXT_SCORE=0.01
RAG_DOCUMENT_MIN_SCORE=0.05
RAG_DOCUMENT_RELATIVE_CUTOFF=0.55
RAG_DOCUMENT_MIN_COMPETITIVE_SIGNAL=0.25
...
Если `OPENWEBUI_DOCUMENT_LOADER_API_KEY` пустой, `/process` использует
`OPENAI_COMPAT_API_KEY`. Лимиты `RAG_DOCUMENT_MIN_*` отсекают документы, у которых нет
реального совпадения с темой; `RAG_DOCUMENT_RELATIVE_CUTOFF` дополнительно убирает слабый
хвост относительно лучшего результата. Отдельный vector/text-канал может сохранить
документ только после `RAG_DOCUMENT_MIN_COMPETITIVE_SIGNAL`, поэтому случайный единственный
слабый кандидат не объявляется «лучшим» в своём канале. Это не влияет на запрос полного
каталога.
...
Отдельно проверить загруженный заново DOCX с автоматическими номерами:

```text
попросить назвать точный номер известного пункта
попросить раскрыть оба ранее названных подпункта
убедиться, что ответ не ссылается на несуществующий номер
```
